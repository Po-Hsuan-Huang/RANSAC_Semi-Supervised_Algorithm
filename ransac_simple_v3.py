...
1. [v.2]Iterative training of the MLP:
...
3. [v.2]Supports multiple epochs :
...
def controversial(preds):
    indx =[]
    preds = np.asarray(preds)
    for n in range(len(preds[0])) : 
        pred, cnts = np.unique(preds[:,n], return_counts = True)
        if np.max(cnts) < 7 : # no censensus on search data point  
            indx.append(n)
...
def max_count_pool(X, preds, num_voters):
    """
    pool the most voted prediciton from the estimators (absolute majority vote)
    ...
    x_consensus, y_consensus = unanimus_consensus(X, preds)
    ...
    if len(x_consensus) > 0 and len(y_consensus) > 0 :
        ...
        return majority_vote(X, preds, num_voters)
...
class RANSAC(object):
    ...
    def backward_pass(self, X, y):
        """
        please run self.fit() before this 
        """        
        
        for _ in range(self.num_iter):  # remvoe the controversial data from training set     
            
            indx= self._distill(self.Xtrain_epoch, self.Ytrain_epoch)
            ...
            for i in indx:
                ...
                self.Xtrain_epoch = np.concatenate((self.Xtrain_epoch[:i], self.Xtrain_epoch[i+1:]), axis = 0)
                self.Ytrain_epoch = np.concatenate((self.Ytrain_epoch[:i], self.Ytrain_epoch[i+1:]), axis = 0)
